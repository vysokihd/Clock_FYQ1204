...
void McuInit()
{
	...
	//ВЫХОДЫ
	DDRB |= SEG_MASK;				//Порт сегментов - выход
	DDRC |= COMMON_MASK;				//Порт цифр - выход
	DDRD |= (1 << DOTS_PIN);			//Порт точек - выход
	DDRB |= (1 << 1);
	//PORTB |= (1 << 1);
	//ВХОДЫ
	...
	TCCR1B |= (0 << CS02) | (1 << CS01) | (0 << CS00);	//выбор делителя 8 и запуск таймера
	OCR1A = 0xff;
	...
	TCCR2 |= (0 << CS02) | (1 << CS01) | (1 << CS00);		//выбор делителя 32 (и запуск таймера)
